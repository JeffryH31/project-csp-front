...
const ScheduleForm = ({
  ...
  const removeDate = (index) =>
    setShowDates(showDates.filter((_, i) => i !== index));
      ...
      {isEditing ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <DatePicker
              selected={
                formData.show_date ? new Date(formData.show_date) : null
              }
              onChange={(date) =>
                setFormData((prev) => ({
                  ...prev,
                  show_date: format(date, "yyyy-MM-dd"),
                }))
              }
              customInput={
                <CustomFormInput
          ...
          <div>
            ...
            <DatePicker
              selected={
                formData.show_time
                  ? new Date(`1970-01-01T${formData.show_time}`)
                  : null
              }
              onChange={(date) =>
                setFormData((prev) => ({
                  ...prev,
                  show_time: format(date, "HH:mm"),
                }))
              }
              customInput={
                <CustomFormInput
              ...
              {showDates.map((date, i) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="ml-1.5 p-0.5 rounded-full text-blue-500 hover:bg-blue-200"
                  >
          ...
          <div className="p-4 border border-slate-200 rounded-lg bg-slate-50">
            ...
            <div className="flex items-center">
              <DatePicker
                ...
                customInput={
                  <CustomFormInput
                    icon={<ClockIcon className="h-5 w-5 text-gray-400" />}
                    placeholder="Pick a time to add"
                  />
                }
                showTimeSelect
                showTimeSelectOnly
              ...
              <button
                type="button"
                onClick={addTime}
                className="ml-2 px-3 py-2 bg-teal-100 text-teal-700 rounded-md hover:bg-teal-200"
              >
                <PlusIcon className="h-5 w-5" />
              </button>
            </div>
            <div className="flex flex-wrap gap-2 mt-3 empty:mt-0">
              {showTimes.map((time, i) => (
                <span
                  key={i}
                  className="inline-flex items-center px-2.5 py-1 rounded-full text-sm font-medium bg-teal-100 text-teal-800"
                >
                  {time}
                  <button
                    type="button"
                    onClick={() => removeTime(i)}
                    className="ml-1.5 p-0.5 rounded-full text-teal-500 hover:bg-teal-200"
                  >
                    <XMarkIcon className="h-3 w-3" />
                  </button>
                </span>
