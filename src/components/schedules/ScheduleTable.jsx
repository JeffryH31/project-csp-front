...
const ScheduleTable = ({ schedules, movies, cinemas, onEdit, onDelete }) => {
  ...
  const commonInputClass =
    "appearance-none pl-10 pr-10 p-2.5 w-full border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500";
    ...
    <div className="bg-white p-6 rounded-xl shadow-lg">
          ...
          <thead className="bg-slate-50">
            <tr>
              ...
              >
                ...
                <SortIcon
                  direction={
                    sortConfig.key === "movie" ? sortConfig.direction : "none"
                  }
                />
              </th>
              ...
              <th
                className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider cursor-pointer"
                onClick={() => requestSort("show_date")}
              >
                Showtime{" "}
                <SortIcon
                  direction={
                    sortConfig.key === "show_date"
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-slate-800">
                      {schedule.cinema?.name || "N/A"}
                    </div>
                    <div className="text-xs text-slate-500">
                      {schedule.cinema?.location || "N/A"} - Studio{" "}
                      {schedule.studio_number}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="font-semibold text-sm text-slate-800">
                      {format(
                        new Date(schedule.show_date),
                        "eeee, d MMMM yyyy",
                        { locale: id }
                      )}
                    </div>
                    <div className="text-sm text-slate-500">
                      {schedule.show_time.substring(0, 5)} WIB
                    </div>
                  </td>
      ...
      <div className="flex justify-between items-center mt-6">
        <span className="text-sm text-gray-500">
          ...
          to{" "}
          <span className="font-semibold text-gray-700">
            {Math.min(currentPage * itemsPerPage, sortedSchedules.length)}
          </span>{" "}
          of{" "}
          <span className="font-semibold text-gray-700">
