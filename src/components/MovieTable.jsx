        ...
        <div className="bg-white p-6 rounded-xl shadow-lg">
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center space-x-2">
                    <label htmlFor="itemsPerPage" className="text-sm text-gray-500">Show</label>
                    <select
                        id="itemsPerPage"
                        value={itemsPerPage}
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full">
                    <thead className="bg-slate-50">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Movie</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider cursor-pointer" onClick={() => requestSort('duration')}>
                    ...
                    <tbody className="bg-white">
                        {paginatedMovies.length > 0 ? (
                            paginatedMovies.map((movie, index) => (
                                <tr key={movie.id} className="border-b border-gray-200 hover:bg-slate-50 transition-colors">
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center">
                                            <div className="flex-shrink-0 h-16 w-12">
                                                {movie.poster_url ?
                                                    <img src={`${IMAGE_BASE_URL}${movie.poster_url}`} alt={movie.title} className="h-16 w-12 object-cover rounded-md shadow" /> :
                                            ...
                                            <div className="ml-4">
                                                <div className="text-sm font-bold text-slate-900">{movie.title}</div>
                                                <div className="text-xs text-slate-500 mt-1 px-2 py-0.5 bg-slate-100 rounded-full inline-block">{movie.genre}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td className="px-6 py-4">
                                        <p className="text-sm text-gray-500 max-w-xs">{truncateText(movie.description, 60)}</p>
                                    </td>
                            ...
                            <tr>
                                <td colSpan="4" className="text-center py-12">
                                    ...
                                    <h3 className="mt-2 text-sm font-semibold text-gray-800">No movies found</h3>
            ...
            <div className="flex justify-between items-center mt-6">
                <span className="text-sm text-gray-500">
                    Showing <span className="font-semibold text-gray-700">{paginatedMovies.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}</span> to <span className="font-semibold text-gray-700">{Math.min(currentPage * itemsPerPage, sortedMovies.length)}</span> of <span className="font-semibold text-gray-700">{sortedMovies.length}</span> results
                </span>
